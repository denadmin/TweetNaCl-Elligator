...
int main() {
 ...
 for (int i = 0; i < sizeof(vectors)/sizeof(struct TestVectors); i++)  {
    u8 test_repr[32] = {0};
    u8 test_pub[32] = {0};
 	
    int valid = crypto_box_keypair_elligator(test_pub, test_repr, vectors[i].priv);
    printf("%d:",i);
    if(vectors[i].valid != valid) {
        printf("vaild err; ");
    }else{
        printf("vaild ok; ");	
    }
    if(memcmp(test_pub, vectors[i].pub, 32)!=0){
        printf("pubic failed; ");
    }else{
        printf("public ok; ");     
    }
    if (valid) {
      if(memcmp(test_repr, vectors[i].repr, 32)!=0){
	printf("repr failed; ");
	}else{
	printf("repr ok; ");
	}
    ...
    if(valid){
	u8 pub[32] = {0};
	decode_repr(pub,test_repr);
	if(memcmp(vectors[i].pub,pub,32)!=0){
		printf("repr decode failed;");
	}else{
		printf("repr decode ok;");
	}
    }
    printf("\n");
 }
